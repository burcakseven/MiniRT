...
t_virtural_canvas make_virtural_canvas(t_camera camera) {
  ...
  canvas.norm_j =
      vec3_scale(canvas.norm_j, vec3_length(canvas.norm_i) / ASPECT_RATIO);
  ...
  canvas.v_unit_h =
      vec3_scale(vec3_subtract(canvas.canva_h.dir, canvas.canva_h.orig),
                 (double)1 / WIDTH);
  canvas.v_unit_v =
      vec3_scale(vec3_subtract(canvas.canva_v.dir, canvas.canva_v.orig),
                 (double)1 / HEIGHT);
  return (canvas);
}
